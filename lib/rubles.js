(function() {
  ...
  var parseNumber = function(number, count, options) {
    var first;
    var second;
    var numeral = '';
    ...
    if (count === 0 && options.nominal) {
      switch (options.currCode) {
        case 'BYN': {
  ...
  var parseDecimals = function(number, options) {
    var text = options.nominal ? plural(number, ['копейка', 'копейки', 'копеек']) : '';
  ...
  var rubles = function(number, opt) {
    ...
    if (options.decimals && decimals) {
      numeral = numeral + parseDecimals(toFloat(decimals), options);
    }
    
    numeral = numeral.replace(/\s+/g, ' ').trim();
